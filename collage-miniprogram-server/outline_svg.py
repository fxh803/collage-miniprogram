...
def insert_point_to_longest_segment(points,num_segments=20):
    ...
    # 递归调用直到插入到8个点
    return insert_point_to_longest_segment(new_points)
...
def uniform_size(svg_dir,save_dir,fixed_size=80,margin=10,index=0):
    ...
    pydiffvg.save_svg(f"{save_dir}/outline_files/uniform_{index}.svg",
                        100,
                        100,
                        shapes,
                        shape_groups)
...
def uniform_image_size(raster_img_dir,mask_img_dir,img_size,save_dir,index):
    ...
    if image.shape[1] >= 500:
        image = image[:,:500-abs(margin-x)]
        raster_img = raster_img[:,:500-abs(margin-x)]
    if image.shape[0] >= 500:
        image = image[:500-abs(margin-y),:]
        raster_img = raster_img[:500-abs(margin-y),:]
    ...
    if image.shape[0] < 500 and image.shape[1] < 500:
        ...
        mask_image = cv2.copyMakeBorder(image, margin-y, 500-image.shape[0]-(margin-y), margin-x, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        raster_image = cv2.copyMakeBorder(raster_img, margin-y, 500-image.shape[0]-(margin-y), margin-x, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
    elif image.shape[0] >= 500 and image.shape[1] < 500:
        mask_image = cv2.copyMakeBorder(image, margin-y, 0, margin-x, 500-image.shape[1]-(margin-x), cv2.BORDER_CONSTANT, value=[0, 0, 0])
        raster_image = cv2.copyMakeBorder(raster_img, margin-y, 0, margin-x, 500-image.shape[1]-(margin-x), cv2.BORDER_CONSTANT, value=[0, 0, 0])
    else:
        mask_image = cv2.copyMakeBorder(image, margin-y, 0, margin-x, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        raster_image = cv2.copyMakeBorder(raster_img, margin-y, 0, margin-x, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
...
def outline_svg(args,device,progress_callback,id,stop_ids):
    primitive_files=[]
    ...
    with tqdm(total=len(primitive_files), desc="Processing value", unit="value") as pbar:
        ...
        for i,primitive_file in enumerate(primitive_files):
            ...
            svg_optimize_img(img_size,[path],[path_group],target_img,device,save_svg_path=f"{args.primitive_dir}/outline_files",num_iters=60,svg_i=i+1)
...
def preprocessing(primitive_dir,device,progress_callback=None,id=None,stop_ids=None):
    ...
    parser.add_argument("-pd","--primitive_dir", type=str,default=f"{primitive_dir}",)
    args = parser.parse_args()
    args = load_config(args.config,args)
    outline_svg(args,device=device,progress_callback = progress_callback ,id = id,stop_ids = stop_ids)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-pc", "--primitive_class", type=str, choices=["any_shape_raster","any_shape_svg"], default="any_shape_raster",
                        help="Specify the primitive class (any_shape_raster,any_shape_svg).")
    parser.add_argument("-pd","--primitive_dir", type=str,default="",)
    args = parser.parse_args()
    args = load_config(args.config,args)
