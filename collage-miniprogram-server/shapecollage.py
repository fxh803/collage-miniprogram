...
class ShapeCollage:
    ...
    def __init__(self,args: omegaconf.DictConfig,device,progress_callback,stop_ids) -> None:
        self.stop_ids = stop_ids
        self.process = args.save_name
        self.device = device
        self.progress_callback = progress_callback
        init_diffvg(self.device)

        self.shape_class = args.shape_class
        self.primitive_multiple = args.primitive_multiple
        ...
        self.weights_list = self.weights_list*self.primitive_multiple
        ...
        if self.primitive_class not in ["simple_shape","photo_collage","wordle"]:
            self.primitive_json_list = [f"{args.primitive_dir}/outline_files/{i+1}.json" for i in range(int(self.primitive_num/self.primitive_multiple))]
            self.primitive_json_list *= self.primitive_multiple
            self.primitive_mask_list = [f"{args.primitive_dir}/outline_files/{i+1}.png" for i in range(int(self.primitive_num/self.primitive_multiple))]
            self.primitive_mask_list *= self.primitive_multiple
            self.primitive_outline_list = [f"{args.primitive_dir}/outline_files/outline_{i+1}.svg" for i in range(int(self.primitive_num/self.primitive_multiple))]
            self.primitive_outline_list *= self.primitive_multiple
            if self.primitive_class != "any_shape_raster":
                self.primitive_uniform_list = [f"{args.primitive_dir}/outline_files/uniform_{i+1}.svg" for i in range(int(self.primitive_num/self.primitive_multiple))]
            if self.primitive_class == "any_shape_raster":
                self.primitive_uniform_list = [f"{args.primitive_dir}/outline_files/uniform_{i+1}.png" for i in range(int(self.primitive_num/self.primitive_multiple))]
            self.primitive_uniform_list *= self.primitive_multiple
    ...
    def shape_collage(self):
        ...
        if self.primitive_class == "simple_shape":
            ...
            self.photo_list = self.photo_list*self.primitive_multiple
        ...
        with tqdm(total=self.num_iters, desc="Processing value", unit="value") as pbar:
            for epoch in range(self.num_iters):
                # print(self.stop_ids)
                if self.process in self.stop_ids:
                    print("stop!!!")
                    self.stop_ids.remove(self.process)
                    break
                    
                if self.progress_callback:
                    self.progress_callback(self.process, 1, int(((epoch+1)/self.num_iters)*100))
                ...
                if self.shape_class == "closed":
                    if self.is_global_size:
                        ...
                        loss_area_proportional = weight_loss(size_tensor,target_area_list,scale=5e1)
                        loss_uniform = 0
                    loss_force = 0
                elif self.shape_class == "open":
                    loss_area_proportional=0
                    loss_uniform = 0
                    loss_force = force_loss(pos_tensor,"down")
